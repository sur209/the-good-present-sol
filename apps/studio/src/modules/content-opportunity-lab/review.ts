...
export function createEditorialBrief(
  ...
  return editorialBriefSchema.parse({
    ...
    differentiation: candidate.differentiation ?? candidate.problemSolved,
    plannedSections: candidate.proposedSections,
    productRequirements: candidate.productRequirements ?? candidate.distinctiveProductCategories,
    researchQuestions: [],
    expectedInternalLinks: candidate.closestExistingContentIds,
    relatedContentIds: candidate.closestExistingContentIds,
    evidenceNotes: {
      deterministic: [
        ...candidate.overlapSignals.map(
          (signal) => `${signal.contentId} · ${signal.kind} · ${signal.level}: ${signal.reason}`,
        ),
        ...(candidate.sourceProductIds ?? []).map((id) => `Source product: ${id}`),
        ...(candidate.sourceCategoryIds ?? []).map((id) => `Source category: ${id}`),
        ...(candidate.sourceCoverageSignalIds ?? []).map((id) => `I.0 coverage signal: ${id}`),
      ],
    ...
    risks: candidate.maintenanceImplications ? [candidate.maintenanceImplications] : [],
    status: "draft",
