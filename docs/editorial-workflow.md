...
7. Optionally add manual or future Creators API candidates to the same queue. A later P.3 source must implement the existing Product discovery/source interface and reuse this lifecycle.
8. Select the canonical Product separately for I.2, then return to an originating brief or use **Assign to originating slot**. Assignment reuses the ordinary Goal 2 product-selection function and returns to that exact slot.
9. Review or generate recommendation copy and run preview/readiness checks. Sourcing never marks copy ready and never publishes; an unresolved slot can be editorially ready while its sourcing request remains a Product gap.
