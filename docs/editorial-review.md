...
Reports live in `editorial-data/editorial-reviews/<review-id>.json`, one record per review. They contain stable issue IDs, guide/slot/field locations, issue type and severity, explanations, original/replacement field text, applied timestamps, the fingerprint, provider/model identifiers, and compact usage metrics. They do not duplicate whole guides or retain full prompts/provider traces. Every new report has `status: "completed"` or `status: "failed"`; failed reports carry a compact `failure` diagnostic with a category, safe provider code, attempt count, provider/model, bounded timeout when known, and timestamp. Failed reviews are recorded separately from successful reviews with zero issues, and old reports without the new diagnostic remain readable.

**Feedback editorial** summarizes completed reviews, failed reviews, completed reviews with issues, completed reviews with zero issues, distinct guides with issues, issue counts by type, applied/unapplied suggestions, and the 20 most recent issues. Failed reviews are excluded from issue-rate denominators and issue counts. Counts represent observed issue occurrences: repeated reviews may record the same unresolved problem again. Unapplied means no suggestion was applied through this action, not that a later manual edit failed to resolve the problem.

Timeouts, transient provider/network failures, and invalid structured responses allow at most one technical retry. Authentication, rate-limit, refusal, and other non-transient failures do not retry automatically. Token totals include both calls when usage is available; an em dash means unknown usage, not zero. Request-local usage avoids mixing metrics with concurrent provider calls.
