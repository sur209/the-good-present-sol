...
Product-backed recommendations resolve every outbound destination through the selected stable product ID and the central product catalog. `productDestination(product)` requires a valid Amazon affiliate destination for Amazon Products and never falls back to an ordinary Amazon `productUrl`; non-Amazon Products retain the valid affiliate-first, ordinary-URL fallback. The guide record never stores a merchant URL. An unresolved gift idea has no Product lookup and renders no Product, merchant, price, or shopping UI.

An affiliate destination is manually controlled editorial data and uses a safe absolute HTTP(S) URL. Amazon public shopping CTAs use only the valid affiliate destination; a resolved Amazon Product without one remains editorially publishable but renders no CTA. Affiliate links open in a new tab with `rel="sponsored nofollow noopener"`. Non-Amazon ordinary product URLs open in a new tab with `rel="nofollow noopener"` and use a `View product at…` label, so a direct merchant link is not presented as an affiliate link. Products without an eligible destination render no merchant CTA. Links are direct external anchors; there is no internal open-redirect route or URL parameter.
